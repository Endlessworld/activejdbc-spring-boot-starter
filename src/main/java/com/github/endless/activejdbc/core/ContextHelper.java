...
public abstract class ContextHelper {
     ...
     * json字符转java对象
     */
    public static <T> T convert(Class<T> clazz, String input) {
        try {
    ...
    public static <K, V> Map<K, V> deLayer(Map<K, V> input) {
        if (input.containsKey("input")) {
            input.putAll(convert(HashMap.class, Convert.toString(input.get("input"))));
        }
        return input;
    }
    ...
    public static boolean existKey(ColumnMetadata metaData, Map input) {
        return (input.containsKey(metaData.getColumnName()) || input.containsKey(metaData.getColumnName().toLowerCase()))
                && (((input.get(metaData.getColumnName()) != null && input.get(metaData.getColumnName()) != ""))
                || ((input.get(metaData.getColumnName().toLowerCase()) != null && input.get(metaData.getColumnName().toLowerCase()) != "")));
    }
    ...
    public static Set getAttributeNames(Class<? extends Model>... modelClass) {
        return Stream.of(modelClass).map(clazz -> ModelDelegate.metaModelOf(clazz).getAttributeNamesSkipGenerated())
                .reduce(new HashSet<>(), ContextHelper::merge);
    }
    ...
    /**
     * 构造mybatis的分页对象 必须先执行查询，在执行count否则分页无效
     */
//    public static <T> Response<PageQuery<T>> helper(Function<List<T>> mapper, Map input) {
//        List<T> rows = queryHelper(mapper, input);
//        long total = PageHelper.count(mapper::apply);
//        return Response.respone(new PageQuery<T>().setRows(rows).setTotal(total));
//    }
//
//    /**
//     * 执行分页查询
//     */
//    public static <T> List<T> queryHelper(Function<List<T>> mapper, Map input) {
//        PaginatorQuery info = analysis(input);
//        PageHelper.startPage(info.getPageNum(), info.getPageSize());
//        PageHelper.orderBy(info.getOrderBy());
//        return mapper.apply();
//    }
     ...
     * json转VO
     */
    public static <T> T toVO(String input, Class<T> clazz) {
        return JSONObject.parseObject(input, clazz);
    }
    ...
    public static <T> String toJson(T input) {
        return JSONObject.toJSONString(input, SerializerFeature.WriteDateUseDateFormat);
    }
    ...
    public static DB openConnection(String modelType) {
        ...
        DynamicRoutingDataSource dataSource = getDataSource(modelType);
        if (dataSource != null) {
            try {
     ...
     * 根据注解打开数据库链接
     */
    public static DynamicRoutingDataSource getDataSource(String modelType) {
        ApplicationContextHelper.setDataSourceKey(modelType);
        return ApplicationContextHelper.getBeanByType(DynamicRoutingDataSource.class);
    }
